...
const configLoader_1 = require("../src/configLoader");
...
async function writeTestConfig(config, filename) {
    const configPath = path_1.default.join(process.cwd(), filename);
    await fs_1.promises.writeFile(configPath, JSON.stringify(config, null, 2), 'utf-8');
    return configPath;
}
async function cleanupTestConfig(filename) {
    try {
        await fs_1.promises.unlink(path_1.default.join(process.cwd(), filename));
    }
    catch (error) {
        // Ignore cleanup errors
    }
}
async function testConfigValidation(testName, config, shouldSucceed) {
    ...
    try {
        ...
        configLoader_1.ConfigLoader.resetInstance();
        const loader = configLoader_1.ConfigLoader.getInstance(filename);
...
async function testAnalyzeNewsIntegration() {
    ...
    try {
        ...
        configLoader_1.ConfigLoader.resetInstance();
        const enabledLoader = configLoader_1.ConfigLoader.getInstance(filename);
        ...
        configLoader_1.ConfigLoader.resetInstance();
        const disabledLoader = configLoader_1.ConfigLoader.getInstance(filename);
        ...
        configLoader_1.ConfigLoader.resetInstance();
        const missingLoader = configLoader_1.ConfigLoader.getInstance(filename);
...
async function runAllTests() {
    ...
    results.push(await testConfigValidation("Valid configuration with analysis enabled", TEST_CONFIG_ENABLED, true));
    results.push(await testConfigValidation("Valid configuration with analysis disabled", TEST_CONFIG_DISABLED, true));
    results.push(await testConfigValidation("Valid configuration without analysis section (backward compatibility)", TEST_CONFIG_MISSING, true));
    // Test invalid configurations
    results.push(await testConfigValidation("Invalid configuration with non-boolean enabled field", TEST_CONFIG_INVALID, false));
    results.push(await testConfigValidation("Invalid configuration with invalid temperature", {
        ...TEST_CONFIG_ENABLED,
        analysis: {
            openrouter: {
                enabled: true,
    ...
    results.push(await testConfigValidation("Invalid configuration with non-string model", {
        ...TEST_CONFIG_ENABLED,
        analysis: {
            openrouter: {
                enabled: true,
